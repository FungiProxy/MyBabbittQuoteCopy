...
def test_length_adder_rules():
    ...
    for rule in rules:
        print(
            f"  {rule['material_code']}: {rule['adder_type']} starting at {rule['first_threshold']}\" = ${rule['adder_amount']}"
        )
    ...
    with SessionLocal() as session:
        ...
        for row in result:
            print(
                f'  {row.material_code}: {row.adder_type} starting at {row.first_threshold}" = ${row.adder_amount}'
            )
