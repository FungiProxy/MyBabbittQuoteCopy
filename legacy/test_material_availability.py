...
from src.core.models import MaterialAvailability, Product
...
def test_material_availability():
    ...
    try:
        ...
        for product_type in product_types:
            ...
            available_materials = ProductService.get_available_materials_for_product(
                db, product_type
            )
            ...
            for material_code in all_materials:
                is_available = ProductService.is_material_available_for_product(
                    db, material_code, product_type
                )
...
def test_pricing_with_material_availability():
    ...
    try:
        ...
        for product_data in test_products:
            existing_product = (
                db.query(Product)
                .filter(Product.model_number == product_data['model_number'])
                .first()
            )
        ...
        for product_type in ['LS2000', 'LS7000/2', 'FS10000']:
            ...
            product = (
                db.query(Product)
                .filter(Product.model_number.startswith(product_type))
                .first()
            )
            ...
            for material_code in test_materials:
                try:
                    ...
                    print(f'  With {material_code} material: {e!s}')
    finally:
        db.close()
