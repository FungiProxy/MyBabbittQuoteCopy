...
def populate_materials(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"Materials table already has {existing_count} records. Skipping population."
        )
        return
...
def populate_standard_lengths(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"StandardLength table already has {existing_count} records. Skipping population."
        )
        return
...
def populate_material_availability(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"MaterialAvailability table already has {existing_count} records. Skipping population."
        )
        return
...
def populate_from_price_list(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"ProductFamily table already has {existing_count} records. Skipping population."
        )
        return
...
def populate_options(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"Option table already has {existing_count} records. Skipping population."
        )
        return
...
def populate_sample_customers(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"Customer table already has {existing_count} records. Skipping population."
        )
        return
...
def remove_obsolete_products(db: Session):
    ...
    for family in obsolete_families:
        logger.info(
            f"Removing obsolete product family: {family.name} - {family.description}"
        )
    ...
    db.commit()
    logger.info(
        f"Removed {len(obsolete_families)} obsolete product families and their variants"
    )
...
def populate_spare_parts(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"SparePart table already has {existing_count} records. Skipping population."
        )
        return
    ...
    for part_data in spare_parts_data:
        ...
        if product_family_id is None:
            logger.warning(
                f"Product family {part_data['product_family']} not found for spare part {part_data['part_number']}"
            )
            continue
...
def populate_connection_options(db: Session):
    ...
    if existing_count > 0:
        logger.info(
            f"ConnectionOption table already has {existing_count} records. Skipping population."
        )
        return
