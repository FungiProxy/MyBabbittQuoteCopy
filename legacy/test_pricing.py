...
from src.core.pricing import calculate_option_price, calculate_product_price
...
def test_product_pricing(db: Session):
    ...
    for model_number, description in test_models:
        ...
        product = (
            db.query(Product)
            .filter(
                Product.model_number.startswith(model_number), Product.material == "S"
            )
            .first()
        )
        ...
        if product:
            ...
            print(
                f'Base configuration (S material, {product.base_length}"): ${product.base_price:.2f}'
            )
