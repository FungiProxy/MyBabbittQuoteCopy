...
from src.core.services import CustomerService, ProductService, QuoteService
...
def test_customer_service():
    ...
    try:
        ...
        print(
            f'Created customer: {customer.name} at {customer.company} (ID: {customer.id})'
        )
        ...
        print(
            f'Updated customer: {updated_customer.name} at {updated_customer.company}'
        )
...
def test_product_service():
    ...
    try:
        ...
        for variant in variants[:3]:  # Show only first 3 variants
            print(
                f'  - {variant.model_number}: {variant.description} (ID: {variant.id})'
            )
        ...
        if variants:
            ...
            if hasattr(product, 'base_length'):
                ...
                print(
                    f'Same product with length {custom_length}" - Price: ${price:.2f}'
                )
...
def test_quote_service():
    ...
    try:
        ...
        print(
            f'Using customer: {customer.name} at {customer.company} (ID: {customer.id})'
        )
        ...
        quote_item = QuoteService.add_product_to_quote(
            db=db, quote_id=quote.id, product_id=product.id, quantity=2
        )
        print(
            f'Added line item: {quote_item.description} - ${quote_item.unit_price:.2f} each'
        )
        ...
        if options:
            ...
            quote_item_option = QuoteService.add_option_to_quote_item(
                db=db, quote_item_id=quote_item.id, option_id=option.id, quantity=1
            )
        ...
        for item in quote.items:
            ...
            if item.options:
                for option_item in item.options:
                    ...
                    print(
                        f'    + {option.name}: {option_item.quantity} x ${option_item.price:.2f}'
                    )
