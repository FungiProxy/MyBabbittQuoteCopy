...
def parse_price_list(price_list_path):
    ...
    with open(price_list_path) as file:
        content = file.read()
    ...
    for section in family_sections:
        ...
        if (
            lines[0].strip().endswith('LEVEL SWITCH')
            or lines[0].strip().endswith('TRANSMITTER')
            or 'FLOW SWITCH' in lines[0]
        ):
            current_family = {
                ...
                'category': (
                    'Level Switch'
                    if 'LEVEL SWITCH' in lines[0]
                    else (
                        'Level Transmitter'
                        if 'TRANSMITTER' in lines[0]
                        else 'Flow Switch' if 'FLOW SWITCH' in lines[0] else 'Other'
                    )
                ),
                'variants': [],
            }
        ...
        if current_family:
            for line in lines:
                ...
                match = re.search(
                    r'([A-Z0-9/]+)-([0-9A-Z]+)-([A-Z]+)-([0-9]+\")\s+\$([0-9,.]+)', line
                )
                if match:
...
def populate_products_from_data(db: Session, product_data):
    ...
    for family_data in product_data:
        ...
        print(
            f"Added family {family.name} with {len(family_data['variants'])} variants"
        )
