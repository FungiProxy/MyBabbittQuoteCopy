...
def suggest_better_category_name(current_name, option_names):
    ...
    if any('voltage' in name.lower() for name in option_names):
        return 'Voltages'
    elif any('connection' in name.lower() or 'npt' in name.lower() or 'flange' in name.lower() for name in option_names):
        return 'Connections'
...
def get_category_reasoning(current_name, option_names):
    ...
    if current_name == 'Electrical':
        ...
        connection_count = sum(1 for name in option_names if any(x in name.lower() for x in ['connection', 'npt', 'flange', 'tri-clamp']))
        total_count = len(option_names)
        if connection_count == total_count:
            return "All options are connection-related, so 'Connections' is more specific"
        else:
