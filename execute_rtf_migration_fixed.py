...
def execute_migration():
    ...
    try:
        print("=== EXECUTING RTF MIGRATION (FIXED) ===")
        ...
        try:
            result = db.execute(text("SELECT insulator_material FROM base_models LIMIT 1"))
            print("  ✓ Columns already exist, skipping column creation")
        except Exception:
            print("  → Columns don't exist, creating them...")
        ...
        print(f"  ✓ Loaded {len(rtf_data)} RTF templates")
        print(f"  ✓ Available keys: {list(rtf_lookup.keys())}")
        
        # Step 3: Update base models using raw SQL
        ...
        for model in base_models:
            ...
            if rtf_item:
                ...
                # Clean the data
                insulator_material = clean_rtf_text(rtf_item.get('insulator_material'))
                insulator_length = rtf_item.get('insulator_length')
                insulator_temp_rating = clean_rtf_text(rtf_item.get('insulator_temp_rating'))
                process_connection = clean_rtf_text(rtf_item.get('process_connection'))
                max_pressure = rtf_item.get('max_pressure')
                housing_type = clean_rtf_text(rtf_item.get('housing_type'))
                housing_ratings = clean_rtf_text(rtf_item.get('housing_ratings'))
                application_notes = clean_rtf_text(rtf_item.get('application_notes'))
                ...
                if special_notes:
                    ...
                    special_notes_text = '; '.join(cleaned_notes) if cleaned_notes else None
                else:
                    special_notes_text = None
                
                # Update using raw SQL to avoid SQLAlchemy model issues
                update_sql = text("""
                    UPDATE base_models 
                    SET insulator_material = :insulator_material,
                        insulator_length = :insulator_length,
                        insulator_temp_rating = :insulator_temp_rating,
                        process_connection = :process_connection,
                        max_pressure = :max_pressure,
                        housing_type = :housing_type,
                        housing_ratings = :housing_ratings,
                ...
                db.execute(update_sql, {
                    ...
                    'housing_ratings': housing_ratings,
                    'application_notes': application_notes,
                    'special_notes': special_notes_text,
                    'model_id': model.id
                })
                
                print(f"  ✓ Updated with RTF data")
                print(f"    Insulator: {insulator_material}")
            else:
        ...
        print("\n=== MIGRATION SUMMARY ===")
        result = db.execute(text("SELECT COUNT(*) as total FROM base_models"))
        total = result.fetchone()[0]
        
        result = db.execute(text("SELECT COUNT(*) as with_data FROM base_models WHERE insulator_material IS NOT NULL"))
        with_data = result.fetchone()[0]
        
        print(f"Base models updated with RTF data: {with_data}/{total}")
        
        # Show which models have data
        result = db.execute(text("SELECT model_number, insulator_material FROM base_models WHERE insulator_material IS NOT NULL"))
        for row in result:
            print(f"  ✓ {row[0]}: {row[1]}")
