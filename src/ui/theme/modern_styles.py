...
def get_button_style(button_type='primary'):
    ...
    Args:
        button_type (str): Type of button ('primary', 'secondary', 'danger')
...
def get_input_style():
    ...
    Returns:
        str: CSS stylesheet for input fields
    """
    return f"""
        QLineEdit {{
            border: 2px solid {COLORS['border_light']};
...
def get_status_badge_style(status_type='info'):
    """
    Generate status badge stylesheet.
    
    Args:
        status_type (str): Type of status ('draft', 'active', 'completed', 'cancelled', 'info')
    
    Returns:
        str: CSS stylesheet for status badges
    """
    styles = {
        'draft': {
...
def get_notification_style(notification_type='info'):
    """
    Generate notification stylesheet.
    
    Args:
        notification_type (str): Type of notification ('success', 'error', 'warning', 'info')
    
    Returns:
        str: CSS stylesheet for notifications
    """
    styles = {
...
def get_font_style(size='base', weight='normal'):
    """
    Generate font style string.
    
    Args:
        size (str): Font size key from FONTS['sizes']
        weight (str): Font weight key from FONTS['weights']
    
    Returns:
        str: Font style string
    """
    font_size = FONTS['sizes'].get(size, FONTS['sizes']['base'])
    font_weight = FONTS['weights'].get(weight, FONTS['weights']['normal'])
    
    return f"font-family: {FONTS['family']}; font-size: {font_size}px; font-weight: {font_weight};"


def get_spacing_style(direction='all', size='md'):
    """
    Generate spacing style string.
    
    Args:
        direction (str): Direction ('all', 'horizontal', 'vertical', 'top', 'right', 'bottom', 'left')
        size (str): Spacing size key from SPACING
    
    Returns:
        str: Spacing style string
    """
    spacing_value = SPACING.get(size, SPACING['md'])
    
    if direction == 'all':
        return f"padding: {spacing_value}px;"
    elif direction == 'horizontal':
        return f"padding-left: {spacing_value}px; padding-right: {spacing_value}px;"
    elif direction == 'vertical':
        return f"padding-top: {spacing_value}px; padding-bottom: {spacing_value}px;"
    else:
        return f"padding-{direction}: {spacing_value}px;" 