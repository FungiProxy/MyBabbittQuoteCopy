...
class BabbittPremiumIntegration:
    ...
    def create_animated_button(text: str, button_type: str = "primary"):
        ...
        if button_type == "primary":
            button.setStyleSheet(f"""
                ...
                QPushButton:pressed {{
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 {BabbittIndustrialPremium.SECONDARY_BLUE},
                        stop:1 {BabbittIndustrialPremium.PRIMARY_BLUE});
                }}
            """)
        else:
            button.setStyleSheet(f"""
                QPushButton {{
                    background: {BabbittIndustrialPremium.STEEL_GRAY};
                    color: {BabbittIndustrialPremium.IRON_GRAY};
                    border: 1px solid {BabbittIndustrialPremium.CHARCOAL_GRAY};
                    border-radius: 8px;
                    padding: 12px 24px;
                    font-size: 14px;
                    font-weight: 500;
                ...
                QPushButton:hover {{
                    background: {BabbittIndustrialPremium.CHARCOAL_GRAY};
                    color: {BabbittIndustrialPremium.PURE_WHITE};
                }}
            """)
    ...
    def _setup_all_animations(main_window):
        ...
        if hasattr(main_window, 'navList'):
            BabbittPremiumIntegration.setup_navigation_animations(main_window.navList)
