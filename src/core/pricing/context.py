...
class PricingContext:
    ...
    def __post_init__(self):
        ...
        self.effective_length_in = self.length_in if self.length_in is not None else self.product.base_length

        # Determine effective material
        material_code = self.material_override_code if self.material_override_code else self.product.material
        self.material = self.db.query(Material).filter(Material.code == material_code).first()
