...
class SparePartService:
    ...
    @staticmethod
    def get_spare_parts_by_family(
        db: Session, product_family_name: str
    ) -> List[SparePart]:
        """
        ...
        product_family = (
            ...
            .filter(ProductFamily.name == product_family_name)
            .first()
        )
        ...
        return (
            db.query(SparePart)
            .filter(SparePart.product_family_id == product_family.id)
            .all()
        )
    ...
    def get_spare_parts_by_category(db: Session, category: str) -> List[SparePart]:
        ...
        Returns:
            List of matching spare parts
        """
        return db.query(SparePart).filter(SparePart.category == category).all()

    @staticmethod
    def get_spare_part_by_part_number(
        db: Session, part_number: str
    ) -> Optional[SparePart]:
        """
        ...
        """
        return db.query(SparePart).filter(SparePart.part_number == part_number).first()
