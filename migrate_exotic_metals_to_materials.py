    ...
    try:
        ...
        for material_data in exotic_materials:
            if material_data['code'] in existing_codes:
                print(f"  ⚠️  {material_data['code']} - {material_data['name']} (already exists)")
            else:
        ...
        for code in exotic_codes:
            for length in standard_lengths:
                ...
                existing = db.query(StandardLength).filter_by(
                    material_code=code, length=length
                ).first()
...
def update_material_options():
    ...
    try:
        ...
        for option in material_options:
            ...
            print(f"  ✅ Updated {option.product_families}: {len(current_choices)} → {len(new_choices)} choices")
