...
def test_get_product_options_by_category(db_session):
    """Test getting options filtered by a specific category."""
    # Setup a family
    family = ProductFamily(
        id=1, name='LS2000', description='Test Family', category='Test'
    )
    db_session.add(family)
    ...
    opt2 = Option(
        id=2, name='Test Feature', category='Feature', product_families='LS2000'
    )
    db_session.add_all([family, opt1, opt2])
    db_session.commit()

    # Create associations
    assoc1 = ProductFamilyOption(product_family_id=1, option_id=1)
    assoc2 = ProductFamilyOption(product_family_id=1, option_id=2)
    db_session.add_all([assoc1, assoc2])
    db_session.commit()

    # Test fetching only 'Connections'
    options = ProductService.get_product_options(db_session, family_name='LS2000')
    connection_options = [opt for opt in options if opt.category == 'Connections']

    assert len(connection_options) == 1
    assert connection_options[0].name == 'Test Connection'
    assert connection_options[0].category == 'Connections'


def test_get_product_options_empty(db_session):
    # Should return empty list if no options for a family
    options = ProductService().get_product_options(db_session, 'NOFAM')
    assert options == []
