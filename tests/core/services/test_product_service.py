...
def test_get_product_families(db_session):
    family = ProductFamily(name="LS2000", description="Level Switch Family", category="Level Switch")
...
def test_get_variants_for_family(db_session):
    ...
    db_session.add(family)
    db_session.commit()
    variant = ProductVariant(product_family_id=family.id, model_number="LS2000-115VAC", description="115VAC Variant", base_price=100.0)
...
def test_get_material_options(db_session):
    mo = MaterialOption(product_family="LS2000", material_code="S", display_name="316 Stainless Steel", base_price=10.0, is_available=1)
    db_session.add(mo)
    db_session.commit()
...
def test_get_connection_options(db_session):
    co = ConnectionOption(type="Flange", rating="150#", size='2"', price=75.0, product_families="LS2000")
    db_session.add(co)
    db_session.commit()
...
def test_get_additional_options(db_session):
    # Included option
    opt1 = Option(name="Explosion Proof", description="Explosion proof housing", price=250.0, price_type="fixed", category="feature", product_families="LS2000", excluded_products=None)
    # Excluded option
    opt2 = Option(name="Special Option", description="Not for LS2000", price=100.0, price_type="fixed", category="feature", product_families="LS2000", excluded_products="LS2000")
...
def test_get_additional_options_excluded_comma(db_session):
    ...
    opt = Option(name="Special", description="Special", price=100.0, price_type="fixed", category="feature", product_families="LS2000", excluded_products="LS2000,LS3000")
    db_session.add(opt)
    db_session.commit()
    options = ProductService().get_additional_options(db_session, "LS2000")
    assert options == []
...
def test_get_valid_options_for_selection(db_session):
    ...
    variants = [
        ProductVariant(product_family_id=family.id, model_number="LS2000-S-115VAC", description="S/115VAC", base_price=100.0, material="S", voltage="115VAC"),
        ProductVariant(product_family_id=family.id, model_number="LS2000-S-24VDC", description="S/24VDC", base_price=100.0, material="S", voltage="24VDC"),
        ProductVariant(product_family_id=family.id, model_number="LS2000-H-115VAC", description="H/115VAC", base_price=100.0, material="H", voltage="115VAC"),
        ProductVariant(product_family_id=family.id, model_number="LS2000-H-24VDC", description="H/24VDC", base_price=100.0, material="H", voltage="24VDC"),
        ProductVariant(product_family_id=family.id, model_number="LS2000-U-24VDC", description="U/24VDC", base_price=100.0, material="U", voltage="24VDC"),
    ]
    ...
    result = service.get_valid_options_for_selection(db_session, family.id, {'material': 'S'})
    assert set(result['voltage']) == {'115VAC', '24VDC'}
    # Select material 'U': only voltage '24VDC' should be available
    result = service.get_valid_options_for_selection(db_session, family.id, {'material': 'U'})
    assert set(result['voltage']) == {'24VDC'}
    # Select voltage '115VAC': only materials with that voltage
    result = service.get_valid_options_for_selection(db_session, family.id, {'voltage': '115VAC'})
    assert set(result['material']) == {'S', 'H'}
    # Select both: only one variant should match, so no further options
    result = service.get_valid_options_for_selection(db_session, family.id, {'material': 'U', 'voltage': '24VDC'})
    assert result == {} 