#!/usr/bin/env python3
"""
Script to extract text from all Word template PNG images using OCR.
This will process all template images and output structured data.
"""
...
def extract_sections_from_text(text, template_info):
    """Extract specific sections from the template text"""
    lines = text.split('\n')
    ...
    for line in lines:
        ...
        # Identify section headers and content
        if any(word in line_lower for word in ['configuration', 'specifications']):
            current_section = 'specifications'
        elif any(word in line_lower for word in ['housing', 'enclosure', 'nema']):
            current_section = 'housing'
        elif any(word in line_lower for word in ['insulator', 'probe']):
            current_section = 'insulator'
        elif any(word in line_lower for word in ['pressure', 'psi', 'bar']):
            current_section = 'pressure_info'
        elif any(word in line_lower for word in ['material', 'stainless', 'halar', 'teflon']):
            current_section = 'material_notes'
        elif any(word in line_lower for word in ['application', 'note', 'designed']):
            current_section = 'application_notes'
        elif any(word in line_lower for word in ['warranty', 'year']):
...
def main():
    ...
    for png_file in png_files:
        ...
        sections = extract_sections_from_text(full_text, template_info)
    ...
    extracted_data['housing_options'] = list(extracted_data['housing_options'])
    extracted_data['pressure_ratings'] = list(extracted_data['pressure_ratings'])
    ...
    print("\nNext steps:")
    print("1. Review extracted_template_data/extraction_summary.txt")
    print("2. Check individual .txt files for detailed content")
    print("3. Use full_extraction.json for database seeding")
