...
def main():
    ...
    for fam in families:
        ...
        if not pf:
            pf = ProductFamily(name=fam['name'], description=fam.get('base_model', {}).get('description', ''), category=None)
            db.add(pf)
            db.commit()
            db.refresh(pf)
        ...
        for opt in fam.get('options', []):
            if opt['name'].lower() == 'voltage':
                for v in opt.get('choices', []):
                    exists = db.query(VoltageOption).filter_by(product_family_id=pf.id, voltage=v).first()
                    if not exists:
                        db.add(VoltageOption(product_family_id=pf.id, voltage=v, is_available=1))
            if opt['name'].lower() == 'material':
                for m in opt.get('choices', []):
                    exists = db.query(MaterialOption).filter_by(product_family_id=pf.id, material_code=m).first()
                    if not exists:
                        db.add(MaterialOption(product_family_id=pf.id, material_code=m, display_name=m, base_price=opt.get('adders', {}).get(m, 0), is_available=1))
            # Import all options into Option table
            exists = db.query(Option).filter_by(name=opt['name'], product_families=fam['name']).first()
            if not exists:
                db.add(Option(
                    name=opt['name'],
