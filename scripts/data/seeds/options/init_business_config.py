...
def init_business_config():
    ...
    try:
        ...
        for option in options:
            ...
            if not exists:
                print(f"Adding option: {option.name} (category: {option.category}) for families: {option.product_families}")
                db.add(option)
            else:
                print(f"Option already exists: {option.name} (category: {option.category})")
