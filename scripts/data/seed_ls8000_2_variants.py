...
def seed_ls8000_2_variants():
    """Seed the database with all possible LS8000/2 product variants."""
    db = SessionLocal()
    try:
        ...
        for material, voltage, probe_type, housing, mounting in itertools.product(materials, voltages, probe_types, housings, mountings):
            ...
            model_number = f"LS8000/2-{voltage}-{material}-10" + probe_suffix[probe_type] + housing_suffix[housing]
            description = f"LS 8000/2 level switch, {voltage}, 10\" {material} probe, {probe_type}, {housing}, {mounting} mounting"
