...
def update_material_availability():
    ...
    try:
        for family_name, available_materials in MATERIAL_AVAILABILITY.items():
            ...
            options = db.query(MaterialOption).filter_by(product_family_id=family.id).all()
            ...
            for option in options:
                ...
                if option.is_available != (1 if is_available else 0):
                    ...
                    print(f"  [UPDATE] {option.material_code} ({option.display_name}): {'Available' if is_available else 'Unavailable'}")
