...
def print_options():
    ...
    for fam in families:
        ...
        conn_labels = [f"{c['type']} {c['size']} {c['rating']}" for c in connection] if connection else 'None'
        print(f"  Voltage Options: {[v['voltage'] for v in voltage] if voltage else 'None'}")
        print(f"  Material Options: {[m['display_name'] for m in material] if material else 'None'}")
        print(f"  Connection Options: {conn_labels}")
        print(f"  Additional Options: {[a['name'] for a in additional] if additional else 'None'}")
    db.close()
