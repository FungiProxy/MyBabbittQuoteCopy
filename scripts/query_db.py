...
def print_materials(db):
    ...
    print(
        f"{'Code':<5} {'Name':<25} {'Base Length':<12} {'Length Adder':<15} {'Special Rules'}"
    )
    print('-' * 80)

    for material in materials:
        length_adder = (
            f'${material.length_adder_per_inch}/inch'
            if material.length_adder_per_inch
            else (
                f'${material.length_adder_per_foot}/foot'
        ...
        if material.has_nonstandard_length_surcharge:
            special_rules.append(
                f'${material.nonstandard_length_surcharge} surcharge for non-standard length'
            )
        ...
        print(
            f'{material.code:<5} {material.name:<25} {material.base_length:<12} {length_adder:<15} {special_rules_str}'
        )
...
def print_standard_lengths(db):
    ...
    for material_code, lengths in lengths_by_material.items():
        print(
            f"{material_code} standard lengths: {', '.join(str(int(length)) if length.is_integer() else str(length) for length in sorted(lengths))}"
        )
...
def print_product_variants(db):
    ...
    print(
        f"{'Model':<25} {'Material':<10} {'Voltage':<10} {'Base Length':<12} {'Base Price'}"
    )
    print('-' * 80)

    for variant in variants:
        print(
            f'{variant.model_number:<25} {variant.material:<10} {variant.voltage:<10} {variant.base_length:<12} ${variant.base_price:.2f}'
        )
...
def print_options(db):
    ...
    for option in options:
        ...
        print(
            f'{option.name:<30} {option.category:<15} {price_str:<15} {compatible_with}'
        )
...
def print_customers(db):
    ...
    for customer in customers:
        print(
            f"{customer.name:<20} {customer.company or 'N/A':<25} {customer.email or 'N/A':<30} {customer.phone or 'N/A'}"
        )
...
def print_material_availability(db):
    ...
    for product_type in product_types:
        ...
        availability_records = (
            db.query(MaterialAvailability)
            .filter(MaterialAvailability.product_type == product_type)
            .all()
        )

        available = [r.material_code for r in availability_records if r.is_available]
        unavailable = [
            r.material_code for r in availability_records if not r.is_available
        ]
