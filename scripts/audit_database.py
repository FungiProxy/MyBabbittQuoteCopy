...
def audit_database():
    ...
    try:
        ...
        print(
            f" -> DATABASE ERROR: Could not query database. Please ensure it exists and is accessible."
        )
        print(f"    Details: {e}")
        return
    ...
    if not missing_families and not extra_families:
        ...
        if missing_families:
            print(
                f" -> MISSING FROM DB ({len(missing_families)}): {', '.join(sorted(list(missing_families)))}"
            )
        if extra_families:
            print(
                f" -> EXTRA IN DB ({len(extra_families)}): {', '.join(sorted(list(extra_families)))}"
            )

    # --- Product Variants (grouped by family) ---
    print("\n--- Product Variants ---")
    all_families = sorted(list(expected_families.union(db_families)))
    for family in all_families:
        ...
        if not missing_variants and not extra_variants:
            ...
            if missing_variants:
                ...
                for variant in sorted(list(missing_variants))[:5]:  # Show first 5
                    print(f"        - {variant}")
                if len(missing_variants) > 5:
                    print(f"        - ... and {len(missing_variants) - 5} more.")
            if extra_variants:
                print(f"    -> EXTRA IN DB ({len(extra_variants)}):")
                for variant in sorted(list(extra_variants))[:5]:  # Show first 5
                    print(f"        - {variant}")
