...
def check_and_create_variants():
    ...
    try:
        ...
        materials = db.query(MaterialOption).filter_by(product_family_id=family.id).all()
        print(f"Found {len(materials)} material options:")
        for m in materials:
            print(f"- {m.material_code} ({m.display_name})")

        # Get existing variants
        existing_variants = db.query(ProductVariant).filter_by(product_family_id=family.id).all()
        ...
        for voltage in voltages:
            for material in materials:
                for length in base_lengths:
                    model_number = f"LS7000/2-{voltage}-{material.material_code}-{int(length)}\""
                    
                    # Check if variant already exists
                    if not any(v.model_number == model_number for v in existing_variants):
