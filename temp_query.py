...
REQUIRED_BASE_MODEL_FIELDS = ['model_number', 'description', 'base_price', 'base_length', 'voltage', 'material']
...
def validate_json(path):
    ...
    for i, family in enumerate(families):
        ...
        for opt in options:
            ...
            # Choices
            if 'choices' in opt and (not isinstance(opt['choices'], list) or not opt['choices']):
                errors.append(f"Family '{fname}' option '{opt.get('name')}' has invalid or empty choices.")
            # Adders
            if 'adders' in opt:
                if not isinstance(opt['adders'], dict):
                    errors.append(f"Family '{fname}' option '{opt.get('name')}' adders is not a dict.")
                else:
                    for k, v in opt['adders'].items():
                        if v is not None and not is_number(v):
                            errors.append(f"Family '{fname}' option '{opt.get('name')}' adder for '{k}' is not a number or null.")
            # Notes/rules
            if 'notes' in opt and not isinstance(opt['notes'], str):
                errors.append(f"Family '{fname}' option '{opt.get('name')}' notes is not a string.")
            if 'rules' in opt and not isinstance(opt['rules'], str):
                errors.append(f"Family '{fname}' option '{opt.get('name')}' rules is not a string.")
            ...
            if field in family and not isinstance(family[field], list):
                ...
                for idx, item in enumerate(family[field]):
                    if not isinstance(item, str):
                        errors.append(f"Family '{fname}' {field}[{idx}] is not a string.")
        # Spare parts
        ...
        for part in spare_parts:
            ...
            if 'price' in part and not is_number(part['price']):
                errors.append(f"Family '{fname}' spare part '{part.get('name')}' price is not a number.")
...
if __name__ == '__main__':
    ...
    if errors:
        ...
        for family in families:
            ...
            print(f"  Options: {[opt.get('name') for opt in family.get('options', [])]}")
            print(f"  Spare Parts: {[part.get('name') for part in family.get('spare_parts', [])]}")
            print()
