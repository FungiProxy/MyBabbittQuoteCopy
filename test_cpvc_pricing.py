...
def test_cpvc_pricing():
    ...
    try:
        ...
        print(f"Expected adder: $400.00")

        if abs(applied_adder - 400.0) < 0.01:
            print(f"✅ CORRECT: CPVC adder matches expected value")
        else:
            print(f"❌ INCORRECT: CPVC adder does not match expected value")
            ...
            if opt.get("choices") and isinstance(opt.get("choices"), list):
                for choice in opt.get("choices"):
                    if isinstance(choice, dict) and choice.get("code") == "CPVC":
                        ...
                        print(f"✅ CPVC found in ProductService output")
        ...
        if not cpvc_found:
            print(f"❌ CPVC not found in ProductService output")
